...
const expansionPatterns: Record<string, (sentence: string) => string> = {
  ...
  '見ました。': (sentence: string) => {
    ...
    if (target) {
      const obj = target[1]
      const patterns = [`昨日${obj}を[見|み]ました。`, `初めて${obj}を[見|み]ました。`, `遠くから${obj}を[見|み]ました。`, `みんなで${obj}を[見|み]ました。`]
      ...
      for (const [pattern, transformer] of Object.entries(expansionPatterns)) {
        if (cleanSentence === pattern || cleanSentence.endsWith(pattern)) {
          ...
          if (newSentence !== cleanSentence) {
            ...
            console.log(
              `     新: ${newSentence.replace(/\[[^\]]+\|[^\]]+\]/g, (m) => m.split('|')[0].slice(1))} (${newSentence.replace(/\[[^\]]+\|[^\]]+\]/g, (m) => m.split('|')[0].slice(1)).length}文字)`
            )
            fixedCount++
